...
async function loadPets() {
  ...
  if (error) {
    petsBody.innerHTML = `<tr><td colspan="6">Error: ${error.message}</td></tr>`;
    return;
  }
  ...
  data.forEach((p) => {
    ...
    tr.innerHTML = `
      <td>${p.media_url ? `<a href="${p.media_url}" target="_blank">View</a>` : "—"}</td>
      <td>${p.name || ""}</td>
      <td>$${p.price ?? 0}</td>
      <td>${p.category || ""}</td>
      <td>${p.status || ""}</td>
  ...
  document.querySelectorAll(".del-btn").forEach((btn) => {
    btn.onclick = async () => {
      ...
      if (mediaPath) await supabaseClient.storage.from("pet-media").remove([mediaPath]);
...
document.getElementById("addPetBtn").onclick = async () => {
  ...
  const { data: created, error: insertErr } = await supabaseClient
    .from("pets")
    .insert([{ name, price, category, status, description }])
    .select()
    .single();
